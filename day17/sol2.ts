...
input.forEach((line, x) =>
  line.split("").forEach((value, y) => {
    let currentCoordinate = { x: x, y: y, z: 0, w: 0 };
    if (value === "#") {
...
for (let process = 0; process < 6; process++) {
  let wValues = coordinates.map((coordinate) => coordinate.w);
  ...
  let wLimits = [Math.min(...wValues) - 1, Math.max(...wValues) + 1];
  ...
  for (let w = wLimits[0]; w <= wLimits[1]; w++) {
    for (let z = zLimits[0]; z <= zLimits[1]; z++) {
      for (let x = xLimits[0]; x <= xLimits[1]; x++) {
        for (let y = yLimits[0]; y <= yLimits[1]; y++) {
          const currentCoordinate = { x, y, z, w };
          ...
          const activeNeighbors = neighborsState.filter(
            (value) => value === "#"
          ).length;
          if (currState === "#") {
